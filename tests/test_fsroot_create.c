...
void create_foo_and_bar(char with_trailing_slash)
{
	...
	fd_foo = fsroot_create(foo_path, 1000, 1000, 0100700, O_CREAT | O_RDWR, &err);
	ck_assert_msg(fd_foo >= 0, "fsroot_create(\"foo\") returned %d (err: %d)\n", fd_foo, err);
	fd_bar = fsroot_create(bar_path, 1000, 1000, 0100700, O_RDWR, &err);
...
void release_foo_and_bar(char with_trailing_slash)
{
	int retval;
	struct stat st;
	char foo_path[PATH_MAX], bar_path[PATH_MAX], foo_fullpath[PATH_MAX], bar_fullpath[PATH_MAX];
	...
	retval = fsroot_release(foo_path);
...
START_TEST(test_fsroot_create)
{
	...
	create_foo_and_bar(0);
	release_foo_and_bar(0);
...
{
	int retval = fsroot_init(dir, 1000, 1000, 0040754);
	ck_assert_msg(retval == FSROOT_OK, "fsroot_init() returned %d\n", retval);

	create_foo_and_bar(1);
	release_foo_and_bar(1);
